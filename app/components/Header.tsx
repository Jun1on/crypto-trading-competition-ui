    ...
    <header className="bg-gray-900 text-white py-4 relative">
      <div className="flex justify-center">
        <nav className="flex justify-center space-x-8">
          {pathname !== "/swap" ? (
            // Regular navigation on non-swap pages
            navItems.map((item) => {
            ...
            // Simplified navigation on swap page
            <>
              <Link href="/">
                <span className="px-3 py-2 text-lg font-medium border-b-2 text-gray-300 hover:text-white hover:border-gray-600 border-transparent transition-all duration-200">
                  Dashboard
                </span>
              </Link>
              <Link href="/swap">
