...
export const SimpleModeProvider = ({ children }: { children: ReactNode }) => {
  ...
  // Effect to save state to cookie whenever it changes
  useEffect(() => {
    // Save the current state to the cookie
    Cookies.set("simpleMode", String(isSimpleMode), { expires: 365 }); // Expires in 1 year
  }, [isSimpleMode]); // Run this effect whenever isSimpleMode changes

  const toggleSimpleMode = () => {
    // State update triggers the save effect above
